...
Names of all hub names can be accessed by object `.xena_hosts`:
...
Something, you may want to check data before you query and download data.
...
`getTCGAdata` provide a quite easy download way for TCGA (hg19 version, different from `gdcHub`) datasets, user can specify multiple options to select which data and corresponding file type want to download. Default this function will return a list include `XenaHub` object and selective datasets information. Once you are sure the datasets is exactly what you want, `download` can be set to `TRUE` to download the data.
